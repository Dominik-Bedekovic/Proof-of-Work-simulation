"""Measure reference-machine throughput for each separately counted operation type."""
...
def benchmark_pow(duration=2.0):
    """Time repeated block-header hashes and return hashes per measured second."""
...
def benchmark_tsp_pouw(duration=2.0, size=0):
    """Time fresh complete solves; return visited B&B nodes per second, excluding
    setup.
    """
    ...
    while elapsed < duration:
        ...
        # Instance construction is excluded; this interval measures solver execution.
        start = time.perf_counter()
        computations, _, _, finished = TspFunction.tsp_solver(benchmark_tsp)
        elapsed += time.perf_counter() - start
...
def benchmark_initial_validation(duration=2.0, size=0):
    """Time successful complete-tour checks and return whole checks per second."""
    solved_tsp = TspData(size, True)
    TspFunction.tsp_solver(solved_tsp)
...
def benchmark_branch_validation(duration=2.0, size=0):
    """Time checks of initial branches and count visited validation nodes per second."""
    solved_tsp = TspData(size, True)
    TspFunction.tsp_solver(solved_tsp)
    proposed_cost = solved_tsp.best_cost
    benchmark_tsp = TspData(size, True)
    ...
    while time.perf_counter() - start < duration:
        ...
        if not valid:
            raise RuntimeError("Benchmark branch validation failed.")
        total_computations += computations
...
def _create_proof_benchmark_transcript(size):
    ...
    return (tsp, transcript, sigma, transcript_root)


def benchmark_transcript(duration=2.0, size=10):
    """Measure add_step throughput on preconstructed records; dictionary creation is
    excluded.
    """
    tsp, source_transcript, sigma, transcript_root = _create_proof_benchmark_transcript(
        size
    )

    # Use genuine records, but prepare their dictionaries outside the append timer.
    source_steps = source_transcript.steps
    if not source_steps:
        return 0.0
    computations = 0
    start = time.perf_counter()
    while time.perf_counter() - start < duration:
        ...
        for source_step in source_steps:
            ...
            benchmark_transcript.add_step(data)
            computations += 1
    elapsed = time.perf_counter() - start
    if elapsed <= 0:
        return 0.0
...
def benchmark_hash_validation(duration=2.0, size=10):
    """Measure successful hash-chain checks in checked records per second."""
    tsp, transcript, sigma, transcript_root = _create_proof_benchmark_transcript(size)
    ...
    while time.perf_counter() - start < duration:
        ...
        if not valid:
            raise RuntimeError(
                "Hash validation benchmark generated an invalid transcript."
            )
...
def benchmark_semantic_validation(duration=2.0, size=10):
    """Measure tour/root checks plus complete replay using the simulation's semantic
    units.
    """
    tsp, transcript, transcript_sigma, transcript_root = (
        _create_proof_benchmark_transcript(size)
    )
    proposed_path = tsp.best_path[:]
    proposed_cost = tsp.best_cost
    if not proposed_path:
        raise RuntimeError(
            "Semantic validation benchmark did not generate a TSP solution."
        )
    total_units = 0
    start = time.perf_counter()
    while time.perf_counter() - start < duration:
        ...
        if not hamiltonian_valid:
            raise RuntimeError(
                "Semantic validation benchmark generated an invalid Hamiltonian cycle."
            )
        root_valid = validation._validate_transcript_root(
            tsp, transcript, transcript_sigma, transcript_root, debug=False
        )
        if not root_valid:
            raise RuntimeError(
                "Semantic validation benchmark generated an invalid transcript root."
            )
        semantic_valid, replay_checks, reconstructed_path, reconstructed_cost = (
            validation._validate_transcript_semantics(
                tsp, transcript, proposed_path, proposed_cost, debug=False
            )
        )
        if not semantic_valid:
            raise RuntimeError(
                "Semantic validation benchmark generated an invalid B&B transcript."
            )
        if reconstructed_path != proposed_path:
            raise RuntimeError(
                "Semantic validation benchmark reconstructed the wrong path."
            )
        if reconstructed_cost != proposed_cost:
            raise RuntimeError(
                "Semantic validation benchmark reconstructed the wrong cost."
            )

        # Use the identical composite-unit definition as the simulation.
        total_units += _semantic_validation_units(transcript, replay_checks)
    elapsed = time.perf_counter() - start
    if elapsed <= 0:
        return 0.0
...
def _semantic_validation_units(transcript, replay_checks):
    """Count the same composite setup/replay units used by proof validation."""
    root_children = transcript.root["data"].get("children", [])
    hamiltonian_units = 1
    root_fixed_units = 1
    root_child_units = len(root_children)
    return hamiltonian_units + root_fixed_units + root_child_units + replay_checks
